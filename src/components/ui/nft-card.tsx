...
export function NFTCard({ nftAddress }: { nftAddress: `0x${string}` }) {
    ...
    return (
        <Link to={`/nfts/${nftAddress}`} className="block h-full">
            <div className="border-4 border-[#1A1A2E] p-6 bg-white shadow-[4px_4px_0px_0px_rgba(26,26,46,1)] hover:shadow-[8px_8px_0px_0px_rgba(26,26,46,1)] hover:translate-x-[-2px] hover:translate-y-[-2px] transition-[transform,shadow,opacity,colors] duration-200 group cursor-pointer h-full flex flex-col">
                ...
                <div className="mb-6">
                    <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-sm font-black tabular-nums">{progress.toFixed(2)}%</span>
                    </div>
                ...
                <div className="mt-auto pt-6 border-t-2 border-[#1A1A2E]">
                    ...
                    <button className="w-full mt-4 bg-blue-400 text-white h-12 font-black uppercase text-sm tracking-wider border-4 border-[#1A1A2E] shadow-[4px_4px_0px_0px_rgba(26,26,46,1)] hover:shadow-[6px_6px_0px_0px_rgba(26,26,46,1)] hover:translate-x-[-2px] hover:translate-y-[-2px] transition-[transform,shadow,opacity,colors]">
                        MINT
                    </button>
                </div>
            </div>
